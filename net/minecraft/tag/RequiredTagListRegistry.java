...
public class RequiredTagListRegistry {
   ...
   public static void updateTagManager(TagManager tagManager) {
      REQUIRED_TAG_LISTS.values().forEach((list) -> {
         list.updateTagManager(tagManager);
      });
   }
   ...
   public static Multimap<Identifier, Identifier> getMissingTags(TagManager tagManager) {
      ...
      REQUIRED_TAG_LISTS.forEach((id, list) -> {
         multimap.putAll(id, list.getMissingTags(tagManager));
      });
      return multimap;
   }
   ...
   public static void validateRegistrations() {
      ...
      boolean bl = Stream.of(requiredTagLists).anyMatch((list) -> {
         return !REQUIRED_TAG_LISTS.containsValue(list);
      });
      if (bl) {
