...
public class Sprite implements AutoCloseable {
   ...
   protected Sprite(SpriteAtlasTexture spriteAtlasTexture, Sprite.Info info, int maxLevel, int atlasWidth, int atlasHeight, int x, int y, NativeImage nativeImage) {
      ...
      if (animationResourceMetadata.getFrameCount() > 0) {
         ...
         for(Iterator var15 = animationResourceMetadata.getFrameIndexSet().iterator(); var15.hasNext(); this.frameYs[n] = o) {
            n = (Integer)var15.next();
            if (n >= k * l) {
      ...
      try {
         ...
         crashReportSection.add("Mipmap levels", (Object)maxLevel);
         throw new CrashException(crashReport);
      }
   ...
   public boolean isPixelTransparent(int frame, int x, int y) {
      return (this.images[0].getPixelColor(x + this.frameXs[frame] * this.info.width, y + this.frameYs[frame] * this.info.height) >> 24 & 255) == 0;
   }
