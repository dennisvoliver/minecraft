...
public class TestCommand {
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      ...
      }))).then(((LiteralArgumentBuilder)CommandManager.literal("runfailed").executes((commandContext) -> {
         return method_29411((ServerCommandSource)commandContext.getSource(), false, 0, 8);
      })).then(((RequiredArgumentBuilder)CommandManager.argument("onlyRequiredTests", BoolArgumentType.bool()).executes((commandContext) -> {
         return method_29411((ServerCommandSource)commandContext.getSource(), BoolArgumentType.getBool(commandContext, "onlyRequiredTests"), 0, 8);
      })).then(((RequiredArgumentBuilder)CommandManager.argument("rotationSteps", IntegerArgumentType.integer()).executes((commandContext) -> {
         return method_29411((ServerCommandSource)commandContext.getSource(), BoolArgumentType.getBool(commandContext, "onlyRequiredTests"), IntegerArgumentType.getInteger(commandContext, "rotationSteps"), 8);
      })).then(CommandManager.argument("testsPerRow", IntegerArgumentType.integer()).executes((commandContext) -> {
         return method_29411((ServerCommandSource)commandContext.getSource(), BoolArgumentType.getBool(commandContext, "onlyRequiredTests"), IntegerArgumentType.getInteger(commandContext, "rotationSteps"), IntegerArgumentType.getInteger(commandContext, "testsPerRow"));
      })))))).then(CommandManager.literal("run").then(((RequiredArgumentBuilder)CommandManager.argument("testName", TestFunctionArgumentType.testFunction()).executes((commandContext) -> {
         return executeRun((ServerCommandSource)commandContext.getSource(), TestFunctionArgumentType.getFunction(commandContext, "testName"), 0);
      })).then(CommandManager.argument("rotationSteps", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeRun((ServerCommandSource)commandContext.getSource(), TestFunctionArgumentType.getFunction(commandContext, "testName"), IntegerArgumentType.getInteger(commandContext, "rotationSteps"));
      }))))).then(((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("runall").executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource(), 0, 8);
      })).then(((RequiredArgumentBuilder)CommandManager.argument("testClassName", TestClassArgumentType.testClass()).executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource(), TestClassArgumentType.getTestClass(commandContext, "testClassName"), 0, 8);
      })).then(((RequiredArgumentBuilder)CommandManager.argument("rotationSteps", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource(), TestClassArgumentType.getTestClass(commandContext, "testClassName"), IntegerArgumentType.getInteger(commandContext, "rotationSteps"), 8);
      })).then(CommandManager.argument("testsPerRow", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource(), TestClassArgumentType.getTestClass(commandContext, "testClassName"), IntegerArgumentType.getInteger(commandContext, "rotationSteps"), IntegerArgumentType.getInteger(commandContext, "testsPerRow"));
      }))))).then(((RequiredArgumentBuilder)CommandManager.argument("rotationSteps", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource(), IntegerArgumentType.getInteger(commandContext, "rotationSteps"), 8);
      })).then(CommandManager.argument("testsPerRow", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource(), IntegerArgumentType.getInteger(commandContext, "rotationSteps"), IntegerArgumentType.getInteger(commandContext, "testsPerRow"));
      ...
      })))).then(CommandManager.literal("exportthis").executes((commandContext) -> {
         return method_29413((ServerCommandSource)commandContext.getSource());
   ...
   private static int executePos(ServerCommandSource source, String variableName) throws CommandSyntaxException {
      BlockHitResult blockHitResult = (BlockHitResult)source.getPlayer().raycast(10.0D, 1.0F, false);
   ...
   private static int method_29411(ServerCommandSource serverCommandSource, boolean bl, int i, int j) {
      ...
      if (bl) {
         collection2 = (Collection)TestFunctions.method_29405().stream().filter(TestFunction::isRequired).collect(Collectors.toList());
      } else {
      ...
      if (collection2.isEmpty()) {
         ...
         sendMessage(serverCommandSource, "Rerunning " + collection2.size() + " failed tests (" + (bl ? "only required tests" : "including optional tests") + ")");
         run(serverCommandSource, collection2, i, j);
         return 1;
      }
   }
   ...
   private static void run(ServerCommandSource source, Collection<TestFunction> testFunctions, int i, int j) {
      ...
      testSet.method_29407((gameTest) -> {
         TestFunctions.method_29404(gameTest.method_29403());
      });
   }
   ...
   private static void sendMessage(ServerWorld world, String message, Formatting formatting) {
      world.getPlayers((serverPlayerEntity) -> {
         ...
         serverPlayerEntity.sendSystemMessage(new LiteralText(formatting + message), Util.NIL_UUID);
      });
   }
