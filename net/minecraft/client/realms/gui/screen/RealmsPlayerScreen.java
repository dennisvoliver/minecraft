...
public class RealmsPlayerScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addButton(new ButtonWidget(this.column2_x, row(1), this.column_width + 10, 20, new TranslatableText("mco.configure.world.buttons.invite"), (buttonWidget) -> {
         this.client.openScreen(new RealmsInviteScreen(this.parent, this, this.serverData));
      }));
      this.removeButton = (ButtonWidget)this.addButton(new ButtonWidget(this.column2_x, row(7), this.column_width + 10, 20, new TranslatableText("mco.configure.world.invites.remove.tooltip"), (buttonWidget) -> {
         this.uninvite(this.player);
      }));
      this.opdeopButton = (ButtonWidget)this.addButton(new ButtonWidget(this.column2_x, row(9), this.column_width + 10, 20, new TranslatableText("mco.configure.world.invites.ops.tooltip"), (buttonWidget) -> {
      ...
      this.addButton(new ButtonWidget(this.column2_x + this.column_width / 2 + 2, row(12), this.column_width / 2 + 10 - 2, 20, ScreenTexts.BACK, (buttonWidget) -> {
         this.backButtonClicked();
      }));
      this.titleLabel = (RealmsLabel)this.addChild(new RealmsLabel(new TranslatableText("mco.configure.world.players.title"), this.width / 2, 17, 16777215));
      this.narrateLabels();
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      bufferBuilder.vertex(0.0D, (double)this.height, 0.0D).texture(0.0F, (float)(this.height - i) / 32.0F + 0.0F).color(64, 64, 64, 255).next();
      bufferBuilder.vertex((double)this.width, (double)this.height, 0.0D).texture((float)this.width / 32.0F, (float)(this.height - i) / 32.0F + 0.0F).color(64, 64, 64, 255).next();
      bufferBuilder.vertex((double)this.width, (double)i, 0.0D).texture((float)this.width / 32.0F, 0.0F).color(64, 64, 64, 255).next();
      bufferBuilder.vertex(0.0D, (double)i, 0.0D).texture(0.0F, 0.0F).color(64, 64, 64, 255).next();
      tessellator.draw();
      this.titleLabel.render(this, matrices);
      if (this.serverData != null && this.serverData.players != null) {
         this.textRenderer.draw(matrices, (Text)(new LiteralText("")).append(field_26501).append(" (").append(Integer.toString(this.serverData.players.size())).append(")"), (float)this.column1_x, (float)row(0), 10526880);
      } else {
   ...
   protected void renderMousehoverTooltip(MatrixStack matrices, @Nullable Text text, int mouseX, int mouseY) {
      if (text != null) {
         int i = mouseX + 12;
         int j = mouseY - 12;
         int k = this.textRenderer.getWidth((StringVisitable)text);
   ...
   class InvitedObjectSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry<RealmsPlayerScreen.InvitedObjectSelectionListEntry> {
      ...
      private void renderInvitedItem(MatrixStack matrices, PlayerInfo playerInfo, int x, int y, int mouseX, int mouseY) {
         ...
         RealmsPlayerScreen.this.textRenderer.draw(matrices, playerInfo.getName(), (float)(RealmsPlayerScreen.this.column1_x + 3 + 12), (float)(y + 1), k);
   ...
   class InvitedObjectSelectionList extends RealmsObjectSelectionList<RealmsPlayerScreen.InvitedObjectSelectionListEntry> {
      ...
      public void itemClicked(int cursorY, int selectionIndex, double mouseX, double mouseY, int listWidth) {
         if (selectionIndex >= 0 && selectionIndex <= RealmsPlayerScreen.this.serverData.players.size() && RealmsPlayerScreen.this.operation != RealmsPlayerScreen.PlayerOperation.NONE) {
