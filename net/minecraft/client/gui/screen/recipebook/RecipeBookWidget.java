...
public class RecipeBookWidget extends DrawableHelper implements Drawable, Element, RecipeDisplayListener, RecipeGridAligner<Ingredient> {
   ...
   public void reset(boolean narrow) {
      ...
      Iterator var5 = RecipeBookGroup.method_30285(this.craftingScreenHandler.getCategory()).iterator();
   ...
   public int findLeftEdge(boolean narrow, int width, int parentWidth) {
      ...
      if (this.isOpen() && !narrow) {
         j = 177 + (width - parentWidth - 200) / 2;
      } else {
         j = (width - parentWidth) / 2;
      }
   ...
   private void refreshResults(boolean resetCurrentPage) {
      ...
      list.forEach((recipeResultCollection) -> {
         recipeResultCollection.computeCraftables(this.recipeFinder, this.craftingScreenHandler.getCraftingWidth(), this.craftingScreenHandler.getCraftingHeight(), this.recipeBook);
      });
