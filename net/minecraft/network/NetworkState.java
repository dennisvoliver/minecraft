   ...
   static {
      ...
      for(int var2 = 0; var2 < var1; ++var2) {
         ...
         networkState.packetHandlers.forEach((networkSide, packetHandler) -> {
            packetHandler.getPacketTypes().forEach((class_) -> {
               if (HANDLER_STATE_MAP.containsKey(class_) && HANDLER_STATE_MAP.get(class_) != networkState) {
                  throw new IllegalStateException("Packet " + class_ + " is already assigned to protocol " + HANDLER_STATE_MAP.get(class_) + " - can't reassign to " + networkState);
               } else {
