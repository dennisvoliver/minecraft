...
public class EnchantRandomlyLootFunction extends ConditionalLootFunction {
   ...
   public ItemStack process(ItemStack stack, LootContext context) {
      ...
      if (this.enchantments.isEmpty()) {
         ...
         List<Enchantment> list = (List)Registry.ENCHANTMENT.stream().filter(Enchantment::isAvailableForRandomSelection).filter((enchantment) -> {
            return bl || enchantment.isAcceptableItem(stack);
         }).collect(Collectors.toList());
         if (list.isEmpty()) {
            LOGGER.warn((String)"Couldn't find a compatible enchantment for {}", (Object)stack);
            return stack;
         }
      ...
      return method_26266(stack, enchantment2, random);
   }
