...
public class StuckStingersFeatureRenderer<T extends LivingEntity, M extends PlayerEntityModel<T>> extends StuckObjectsFeatureRenderer<T, M> {
   ...
   protected void renderObject(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, Entity entity, float directionX, float directionY, float directionZ, float tickDelta) {
      ...
      float g = (float)(Math.atan2((double)directionX, (double)directionZ) * 57.2957763671875D);
      float h = (float)(Math.atan2((double)directionY, (double)f) * 57.2957763671875D);
      ...
      matrices.translate(2.5D, 0.0D, 0.0D);
