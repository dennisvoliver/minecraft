         ...
         if (this.isTouchingLiquid(structureWorldAccess, boundingBox)) {
            ...
            for(int i = this.boundingBox.minX; i <= this.boundingBox.maxX; ++i) {
               for(int j = this.boundingBox.minZ; j <= this.boundingBox.maxZ; ++j) {
                  if (this.getBlockAt(structureWorldAccess, i, this.boundingBox.minY - 1, j, boundingBox).isAir() && this.isUnderSeaLevel(structureWorldAccess, i, this.boundingBox.minY - 1, j, boundingBox)) {
   ...
   public static class MineshaftCorridor extends MineshaftGenerator.MineshaftPart {
      ...
      protected boolean addChest(StructureWorldAccess structureWorldAccess, BlockBox boundingBox, Random random, int x, int y, int z, Identifier lootTableId) {
         ...
         if (boundingBox.contains(blockPos) && structureWorldAccess.getBlockState(blockPos).isAir() && !structureWorldAccess.getBlockState(blockPos.down()).isAir()) {
            ...
            ChestMinecartEntity chestMinecartEntity = new ChestMinecartEntity(structureWorldAccess.toServerWorld(), (double)blockPos.getX() + 0.5D, (double)blockPos.getY() + 0.5D, (double)blockPos.getZ() + 0.5D);
      ...
      public boolean generate(StructureWorldAccess structureWorldAccess, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox boundingBox, ChunkPos chunkPos, BlockPos blockPos) {
         if (this.isTouchingLiquid(structureWorldAccess, boundingBox)) {
            ...
            for(t = 0; t < this.length; ++t) {
               x = 2 + t * 5;
               this.generateSupports(structureWorldAccess, boundingBox, 0, 0, x, 2, 2, random);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.1F, 0, 2, x - 1);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.1F, 2, 2, x - 1);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.1F, 0, 2, x + 1);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.1F, 2, 2, x + 1);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.05F, 0, 2, x - 2);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.05F, 2, 2, x - 2);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.05F, 0, 2, x + 2);
               this.addCobwebsUnderground(structureWorldAccess, boundingBox, random, 0.05F, 2, 2, x + 2);
               if (random.nextInt(100) == 0) {
                  this.addChest(structureWorldAccess, boundingBox, random, 2, 0, x - 1, LootTables.ABANDONED_MINESHAFT_CHEST);
               }

               if (random.nextInt(100) == 0) {
                  this.addChest(structureWorldAccess, boundingBox, random, 0, 0, x + 1, LootTables.ABANDONED_MINESHAFT_CHEST);
               }
            ...
            for(t = 0; t <= 2; ++t) {
               for(x = 0; x <= m; ++x) {
                  int v = true;
                  BlockState blockState2 = this.getBlockAt(structureWorldAccess, t, -1, x, boundingBox);
                  if (blockState2.isAir() && this.isUnderSeaLevel(structureWorldAccess, t, -1, x, boundingBox)) {
                     int w = true;
                     this.addBlock(structureWorldAccess, blockState, t, -1, x, boundingBox);
                  }
               }
            }
            ...
            if (this.hasRails) {
               ...
               for(x = 0; x <= m; ++x) {
                  ...
                  if (!blockState4.isAir() && blockState4.isOpaqueFullCube(structureWorldAccess, new BlockPos(this.applyXTransform(1, x), this.applyYTransform(-1), this.applyZTransform(1, x)))) {
   ...
   public static class MineshaftRoom extends MineshaftGenerator.MineshaftPart {
      ...
      public boolean generate(StructureWorldAccess structureWorldAccess, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox boundingBox, ChunkPos chunkPos, BlockPos blockPos) {
         if (this.isTouchingLiquid(structureWorldAccess, boundingBox)) {
            ...
            this.fillWithOutline(structureWorldAccess, boundingBox, this.boundingBox.minX, this.boundingBox.minY, this.boundingBox.minZ, this.boundingBox.maxX, this.boundingBox.minY, this.boundingBox.maxZ, Blocks.DIRT.getDefaultState(), AIR, true);
            this.fillWithOutline(structureWorldAccess, boundingBox, this.boundingBox.minX, this.boundingBox.minY + 1, this.boundingBox.minZ, this.boundingBox.maxX, Math.min(this.boundingBox.minY + 3, this.boundingBox.maxY), this.boundingBox.maxZ, AIR, AIR, false);
            Iterator var8 = this.entrances.iterator();

            while(var8.hasNext()) {
               BlockBox blockBox = (BlockBox)var8.next();
               this.fillWithOutline(structureWorldAccess, boundingBox, blockBox.minX, blockBox.maxY - 2, blockBox.minZ, blockBox.maxX, blockBox.maxY, blockBox.maxZ, AIR, AIR, false);
            }

            this.fillHalfEllipsoid(structureWorldAccess, boundingBox, this.boundingBox.minX, this.boundingBox.minY + 4, this.boundingBox.minZ, this.boundingBox.maxX, this.boundingBox.maxY, this.boundingBox.maxZ, AIR, false);
            return true;
         }
      }
