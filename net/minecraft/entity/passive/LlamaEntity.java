...
public class LlamaEntity extends AbstractDonkeyEntity implements RangedAttackMob {
   ...
   public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable CompoundTag entityTag) {
      ...
      if (entityData instanceof LlamaEntity.LlamaData) {
         j = ((LlamaEntity.LlamaData)entityData).variant;
      } else {
         j = this.random.nextInt(4);
         entityData = new LlamaEntity.LlamaData(j);
      }
   ...
   public Vec3d method_29919() {
      return new Vec3d(0.0D, 0.75D * (double)this.getStandingEyeHeight(), (double)this.getWidth() * 0.5D);
   }
