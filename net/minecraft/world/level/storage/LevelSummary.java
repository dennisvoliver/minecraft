...
public class LevelSummary implements Comparable<LevelSummary> {
   ...
   public MutableText getVersion() {
      return (MutableText)(ChatUtil.isEmpty(this.field_25023.getVersionName()) ? new TranslatableText("selectWorld.versionUnknown") : new LiteralText(this.field_25023.getVersionName()));
   }
   ...
   private Text method_27430() {
      if (this.isLocked()) {
         ...
         MutableText mutableText = this.isHardcore() ? (new LiteralText("")).append((new TranslatableText("gameMode.hardcore")).formatted(Formatting.DARK_RED)) : new TranslatableText("gameMode." + this.getGameMode().getName());
         if (this.hasCheats()) {
            ((MutableText)mutableText).append(", ").append((Text)(new TranslatableText("selectWorld.cheats")));
         }
         ...
         if (this.isDifferentVersion()) {
            mutableText3.append((Text)mutableText2.formatted(this.isFutureLevel() ? Formatting.RED : Formatting.ITALIC));
         } else {
            mutableText3.append((Text)mutableText2);
         }

         ((MutableText)mutableText).append((Text)mutableText3);
