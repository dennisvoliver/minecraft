...
public class EditGameRulesScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addButton(new ButtonWidget(this.width / 2 - 155 + 160, this.height - 29, 150, 20, ScreenTexts.CANCEL, (buttonWidget) -> {
         this.ruleSaver.accept(Optional.empty());
      }));
      this.doneButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width / 2 - 155, this.height - 29, 150, 20, ScreenTexts.DONE, (buttonWidget) -> {
   ...
   public class RuleListWidget extends ElementListWidget<EditGameRulesScreen.AbstractRuleWidget> {
      public RuleListWidget(final GameRules gameRules) {
         ...
         map.entrySet().stream().sorted(Entry.comparingByKey()).forEach((entry) -> {
            ...
            ((Map)entry.getValue()).entrySet().stream().sorted(Entry.comparingByKey(Comparator.comparing(GameRules.Key::getName))).forEach((entryx) -> {
               this.addEntry((EntryListWidget.Entry)entryx.getValue());
            });
         });
      }
      ...
      public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
         ...
         if (this.isMouseOver((double)mouseX, (double)mouseY)) {
            EditGameRulesScreen.AbstractRuleWidget abstractRuleWidget = (EditGameRulesScreen.AbstractRuleWidget)this.getEntryAtPosition((double)mouseX, (double)mouseY);
   ...
   public class BooleanRuleWidget extends EditGameRulesScreen.NamedRuleWidget {
      ...
      public BooleanRuleWidget(final Text name, List<OrderedText> description, final String ruleName, final GameRules.BooleanRule booleanRule) {
         ...
         this.toggleButton = new ButtonWidget(10, 5, 44, 20, ScreenTexts.getToggleText(booleanRule.get()), (buttonWidget) -> {
            ...
            booleanRule.set(bl, (MinecraftServer)null);
