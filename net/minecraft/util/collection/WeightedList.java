...
public class WeightedList<U> {
   ...
   private WeightedList(List<WeightedList.Entry<U>> entries) {
      ...
      this.entries = Lists.newArrayList((Iterable)entries);
   }
      ...
      public static <E> Codec<WeightedList.Entry<E>> createCodec(final Codec<E> codec) {
         return new Codec<WeightedList.Entry<E>>() {
            public <T> DataResult<Pair<WeightedList.Entry<E>, T>> decode(DynamicOps<T> ops, T object) {
               ...
               var10001.getClass();
               return var10000.flatMap(var10001::parse).map((objectx) -> {
                  return new WeightedList.Entry(objectx, dynamic.get("weight").asInt(1));
               }).map((entry) -> {
