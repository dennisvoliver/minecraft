...
public class PointOfInterest {
   ...
   public static Codec<PointOfInterest> createCodec(Runnable updateListener) {
      return RecordCodecBuilder.create((instance) -> {
         ...
         }), Registry.POINT_OF_INTEREST_TYPE.fieldOf("type").forGetter((pointOfInterest) -> {
            return pointOfInterest.type;
         }), Codec.INT.fieldOf("free_tickets").orElse(0).forGetter((pointOfInterest) -> {
            return pointOfInterest.freeTickets;
         }), RecordCodecBuilder.point(updateListener)).apply(instance, (Function4)(PointOfInterest::new));
      });
   }
