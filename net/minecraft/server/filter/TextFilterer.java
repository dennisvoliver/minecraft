...
public class TextFilterer implements AutoCloseable {
   ...
   private void sendJoinOrLeaveRequest(GameProfile gameProfile, URL endpoint, Executor executor) {
      ...
      executor.execute(() -> {
         try {
            ...
            LOGGER.warn((String)"Failed to send join/leave packet to {} for player {}", (Object)endpoint, gameProfile, var5);
         }
      ...
      if (message.isEmpty()) {
         ...
         return CompletableFuture.supplyAsync(() -> {
            try {
               ...
               LOGGER.warn((String)"Failed to validate message '{}'", (Object)message, (Object)var8);
               return Optional.empty();
            }
         }, executor);
      }
   ...
   private JsonObject sendJsonRequest(JsonObject payload, URL endpoint) throws IOException {
      ...
      try {
         if (httpURLConnection.getResponseCode() != 204) {
            try {
               var6 = Streams.parse(new JsonReader(new InputStreamReader(inputStream))).getAsJsonObject();
               return var6;
            } finally {
   ...
   public interface HashIgnorer {
      TextFilterer.HashIgnorer NEVER_IGNORE = (string, i) -> {
         return false;
      };
      TextFilterer.HashIgnorer IGNORE_IF_MATCHES_ALL = (string, i) -> {
   ...
   class Impl implements TextStream {
      ...
      public CompletableFuture<Optional<List<String>>> filterTexts(List<String> texts) {
         List<CompletableFuture<Optional<String>>> list = (List)texts.stream().map((string) -> {
            return TextFilterer.this.filterMessage(this.gameProfile, string, TextFilterer.this.ignorer, this.executor);
         }).collect(ImmutableList.toImmutableList());
         return Util.combine(list).thenApply((listx) -> {
            return Optional.of(listx.stream().map((optional) -> {
