...
public class PlayWithVillagerBabiesTask extends Task<PathAwareEntity> {
   ...
   protected void run(ServerWorld serverWorld, PathAwareEntity pathAwareEntity, long l) {
      ...
      if (livingEntity != null) {
         ...
         if (optional.isPresent()) {
            ...
            this.getVisibleMob(pathAwareEntity).ifPresent((livingEntityx) -> {
               setPlayTarget(pathAwareEntity, livingEntityx);
            });
         }
      }
   }
   ...
   private void setGroundTarget(ServerWorld world, PathAwareEntity entity, LivingEntity unusedBaby) {
      for(int i = 0; i < 10; ++i) {
         ...
         if (vec3d != null && world.isNearOccupiedPointOfInterest(new BlockPos(vec3d))) {
            entity.getBrain().remember(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget(vec3d, 0.6F, 0)));
            return;
         }
      }
   ...
   private static void setPlayTarget(PathAwareEntity entity, LivingEntity target) {
      ...
      brain.remember(MemoryModuleType.INTERACTION_TARGET, (Object)target);
      brain.remember(MemoryModuleType.LOOK_TARGET, (Object)(new EntityLookTarget(target, true)));
      brain.remember(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget(new EntityLookTarget(target, false), 0.6F, 1)));
   }
