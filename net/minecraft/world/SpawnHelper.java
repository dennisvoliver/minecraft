...
public final class SpawnHelper {
   ...
   public static void spawn(ServerWorld world, WorldChunk chunk, SpawnHelper.Info info, boolean spawnAnimals, boolean spawnMonsters, boolean shouldSpawnAnimals) {
      ...
      for(int var8 = 0; var8 < var7; ++var8) {
         SpawnGroup spawnGroup = var6[var8];
         if ((spawnAnimals || !spawnGroup.isPeaceful()) && (spawnMonsters || spawnGroup.isPeaceful()) && (shouldSpawnAnimals || !spawnGroup.isAnimal()) && info.isBelowCap(spawnGroup)) {
            spawnEntitiesInChunk(spawnGroup, world, chunk, (entityType, blockPos, chunkx) -> {
               return info.test(entityType, blockPos, chunkx);
            }, (mobEntity, chunkx) -> {
               info.run(mobEntity, chunkx);
            });
         }
      }
   ...
   private static SpawnSettings.SpawnEntry pickRandomSpawnEntry(ServerWorld serverWorld, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, SpawnGroup spawnGroup, Random random, BlockPos blockPos) {
      ...
      if (spawnGroup == SpawnGroup.WATER_AMBIENT && biome.getCategory() == Biome.Category.RIVER && random.nextFloat() < 0.98F) {
         ...
         return list.isEmpty() ? null : (SpawnSettings.SpawnEntry)WeightedPicker.getRandom(random, list);
      }
   }
   ...
   private static boolean containsSpawnEntry(ServerWorld serverWorld, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, SpawnGroup spawnGroup, SpawnSettings.SpawnEntry spawnEntry, BlockPos blockPos) {
      return method_29950(serverWorld, structureAccessor, chunkGenerator, spawnGroup, blockPos, (Biome)null).contains(spawnEntry);
   }
   ...
   public static void populateEntities(ServerWorldAccess serverWorldAccess, Biome biome, int chunkX, int chunkZ, Random random) {
      ...
      if (!list.isEmpty()) {
         ...
         while(random.nextFloat() < spawnSettings.getCreatureSpawnProbability()) {
            SpawnSettings.SpawnEntry spawnEntry = (SpawnSettings.SpawnEntry)WeightedPicker.getRandom(random, list);
            ...
            for(int p = 0; p < k; ++p) {
               ...
               for(int q = 0; !bl && q < 4; ++q) {
                  ...
                  if (spawnEntry.type.isSummonable() && canSpawn(SpawnRestriction.getLocation(spawnEntry.type), serverWorldAccess, blockPos, spawnEntry.type)) {
                     ...
                     if (!serverWorldAccess.isSpaceEmpty(spawnEntry.type.createSimpleBoundingBox(d, (double)blockPos.getY(), e)) || !SpawnRestriction.canSpawn(spawnEntry.type, serverWorldAccess, SpawnReason.CHUNK_GENERATION, new BlockPos(d, (double)blockPos.getY(), e), serverWorldAccess.getRandom())) {
                        continue;
                     }

                     Entity entity2;
                     try {
                        entity2 = spawnEntry.type.create(serverWorldAccess.toServerWorld());
                     ...
                     if (entity2 instanceof MobEntity) {
                        ...
                        if (mobEntity.canSpawn(serverWorldAccess, SpawnReason.CHUNK_GENERATION) && mobEntity.canSpawn(serverWorldAccess)) {
                           entityData = mobEntity.initialize(serverWorldAccess, serverWorldAccess.getLocalDifficulty(mobEntity.getBlockPos()), SpawnReason.CHUNK_GENERATION, entityData, (CompoundTag)null);
   ...
   public static class Info {
      ...
      private void run(MobEntity entity, Chunk chunk) {
         ...
         if (blockPos.equals(this.cachedPos) && entityType == this.cachedEntityType) {
            ...
            if (spawnDensity != null) {
               f = spawnDensity.getMass();
            } else {
               f = 0.0D;
            }
         }

         this.densityField.addPoint(blockPos, f);
