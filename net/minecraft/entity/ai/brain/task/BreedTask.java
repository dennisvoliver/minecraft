...
public class BreedTask extends Task<AnimalEntity> {
   ...
   protected void keepRunning(ServerWorld serverWorld, AnimalEntity animalEntity, long l) {
      AnimalEntity animalEntity2 = this.getBreedTarget(animalEntity);
      LookTargetUtil.lookAtAndWalkTowardsEachOther(animalEntity, animalEntity2, this.field_23129);
      if (animalEntity.isInRange(animalEntity2, 3.0D)) {
         if (l >= this.breedTime) {
   ...
   private boolean hasBreedTarget(AnimalEntity animal) {
      ...
      return brain.hasMemoryModule(MemoryModuleType.BREED_TARGET) && ((PassiveEntity)brain.getOptionalMemory(MemoryModuleType.BREED_TARGET).get()).getType() == this.targetType;
   }

   private Optional<? extends AnimalEntity> findBreedTarget(AnimalEntity animal) {
      Stream var10000 = ((List)animal.getBrain().getOptionalMemory(MemoryModuleType.VISIBLE_MOBS).get()).stream().filter((livingEntity) -> {
         return livingEntity.getType() == this.targetType;
      }).map((livingEntity) -> {
         return (AnimalEntity)livingEntity;
      });
      animal.getClass();
      return var10000.filter(animal::canBreedWith).findFirst();
   }
}
