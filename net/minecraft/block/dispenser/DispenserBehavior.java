...
public interface DispenserBehavior {
   ...
   static void registerDefaults() {
      ...
      DispenserBlock.registerBehavior(Items.EGG, new ProjectileDispenserBehavior() {
         protected ProjectileEntity createProjectile(World world, Position position, ItemStack stack) {
            return (ProjectileEntity)Util.make(new EggEntity(world, position.getX(), position.getY(), position.getZ()), (eggEntity) -> {
      ...
      DispenserBlock.registerBehavior(Items.SNOWBALL, new ProjectileDispenserBehavior() {
         protected ProjectileEntity createProjectile(World world, Position position, ItemStack stack) {
            return (ProjectileEntity)Util.make(new SnowballEntity(world, position.getX(), position.getY(), position.getZ()), (snowballEntity) -> {
      ...
      DispenserBlock.registerBehavior(Items.EXPERIENCE_BOTTLE, new ProjectileDispenserBehavior() {
         protected ProjectileEntity createProjectile(World world, Position position, ItemStack stack) {
            return (ProjectileEntity)Util.make(new ExperienceBottleEntity(world, position.getX(), position.getY(), position.getZ()), (experienceBottleEntity) -> {
      ...
      DispenserBlock.registerBehavior(Items.SPLASH_POTION, new DispenserBehavior() {
         public ItemStack dispense(BlockPointer blockPointer, ItemStack itemStack) {
            return (new ProjectileDispenserBehavior() {
               protected ProjectileEntity createProjectile(World world, Position position, ItemStack stack) {
                  return (ProjectileEntity)Util.make(new PotionEntity(world, position.getX(), position.getY(), position.getZ()), (potionEntity) -> {
      ...
      DispenserBlock.registerBehavior(Items.CHEST, new FallibleItemDispenserBehavior() {
         public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            BlockPos blockPos = pointer.getBlockPos().offset((Direction)pointer.getBlockState().get(DispenserBlock.FACING));
            List<AbstractDonkeyEntity> list = pointer.getWorld().getEntitiesByClass(AbstractDonkeyEntity.class, new Box(blockPos), (abstractDonkeyEntityx) -> {
               return abstractDonkeyEntityx.isAlive() && !abstractDonkeyEntityx.hasChest();
            });
            Iterator var5 = list.iterator();
            ...
            } while(!abstractDonkeyEntity.isTame() || !abstractDonkeyEntity.equip(499, stack));
      ...
      DispenserBlock.registerBehavior(Items.FLINT_AND_STEEL, new FallibleItemDispenserBehavior() {
         protected ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            if (this.isSuccess() && stack.damage(1, (Random)world.random, (ServerPlayerEntity)null)) {
               stack.setCount(0);
            }
      ...
      DispenserBlock.registerBehavior(Items.GLASS_BOTTLE.asItem(), new FallibleItemDispenserBehavior() {
         ...
         public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            if (blockState.method_27851(BlockTags.BEEHIVES, (abstractBlockState) -> {
   ...
   static void method_27042(BlockPointer blockPointer, Entity entity, Direction direction) {
      entity.updatePosition(blockPointer.getX() + (double)direction.getOffsetX() * (0.5000099999997474D - (double)entity.getWidth() / 2.0D), blockPointer.getY() + (double)direction.getOffsetY() * (0.5000099999997474D - (double)entity.getHeight() / 2.0D) - (double)entity.getHeight() / 2.0D, blockPointer.getZ() + (double)direction.getOffsetZ() * (0.5000099999997474D - (double)entity.getWidth() / 2.0D));
   }
}
