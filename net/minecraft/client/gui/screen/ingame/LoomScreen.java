...
public class LoomScreen extends HandledScreen<LoomScreenHandler> {
   ...
   protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
      ...
      if (this.bannerPatterns != null && !this.hasTooManyPatterns) {
         ...
         matrices.push();
         matrices.translate((double)(i + 139), (double)(j + 52), 0.0D);
         ...
         BannerBlockEntityRenderer.method_29999(matrices, immediate, 15728880, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, this.bannerPatterns);
         matrices.pop();
         immediate.draw();
      ...
      if (this.canApplyDyePattern) {
         ...
         n = ((LoomScreenHandler)this.handler).getSelectedPattern();
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      ...
      if (this.canApplyDyePattern) {
         ...
         for(int l = this.firstPatternButtonId; l < k; ++l) {
            ...
            if (d >= 0.0D && e >= 0.0D && d < 14.0D && e < 14.0D && ((LoomScreenHandler)this.handler).onButtonClick(this.client.player, l)) {
   ...
   static {
      PATTERN_BUTTON_ROW_COUNT = (BannerPattern.COUNT - BannerPattern.field_24417 - 1 + 4 - 1) / 4;
   }
}
