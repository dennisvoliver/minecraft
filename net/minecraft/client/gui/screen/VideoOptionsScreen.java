...
public class VideoOptionsScreen extends GameOptionsScreen {
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      ...
      if (super.mouseClicked(mouseX, mouseY, button)) {
         ...
         if (this.warningManager.shouldWarn()) {
            List<StringVisitable> list = Lists.newArrayList((Object[])(GRAPHICS_WARNING_MESSAGE_TEXT, NEWLINE_TEXT));
            String string = this.warningManager.getRendererWarning();
            if (string != null) {
               list.add(NEWLINE_TEXT);
               list.add((new TranslatableText("options.graphics.warning.renderer", new Object[]{string})).formatted(Formatting.GRAY));
            }

            String string2 = this.warningManager.getVendorWarning();
            if (string2 != null) {
               list.add(NEWLINE_TEXT);
               list.add((new TranslatableText("options.graphics.warning.vendor", new Object[]{string2})).formatted(Formatting.GRAY));
            }

            String string3 = this.warningManager.getVersionWarning();
            if (string3 != null) {
               list.add(NEWLINE_TEXT);
               list.add((new TranslatableText("options.graphics.warning.version", new Object[]{string3})).formatted(Formatting.GRAY));
            }

            this.client.openScreen(new DialogScreen(GRAPHICS_WARNING_TITLE_TEXT, list, ImmutableList.of(new DialogScreen.ChoiceButton(GRAPHICS_WARNING_ACCEPT_TEXT, (buttonx) -> {
   ...
   static {
      GRAPHICS_FABULOUS_TEXT = (new TranslatableText("options.graphics.fabulous")).formatted(Formatting.ITALIC);
      GRAPHICS_WARNING_MESSAGE_TEXT = new TranslatableText("options.graphics.warning.message", new Object[]{GRAPHICS_FABULOUS_TEXT, GRAPHICS_FABULOUS_TEXT});
      GRAPHICS_WARNING_TITLE_TEXT = (new TranslatableText("options.graphics.warning.title")).formatted(Formatting.RED);
      GRAPHICS_WARNING_ACCEPT_TEXT = new TranslatableText("options.graphics.warning.accept");
      GRAPHICS_WARNING_CANCEL_TEXT = new TranslatableText("options.graphics.warning.cancel");
      NEWLINE_TEXT = new LiteralText("\n");
