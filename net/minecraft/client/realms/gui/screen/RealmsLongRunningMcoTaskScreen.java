...
public class RealmsLongRunningMcoTaskScreen extends RealmsScreen implements Errable {
   ...
   public void init() {
      this.task.init();
      this.addButton(new ButtonWidget(this.width / 2 - 106, row(12), 212, 20, ScreenTexts.CANCEL, (buttonWidget) -> {
         this.cancelOrBackButtonClicked();
      }));
   }
   ...
   public void error(Text text) {
      ...
      this.onError();
      this.addButton(new ButtonWidget(this.width / 2 - 106, this.height / 4 + 120 + 12, 200, 20, ScreenTexts.BACK, (buttonWidget) -> {
         this.cancelOrBackButtonClicked();
      }));
   }

   private void onError() {
      Set<Element> set = Sets.newHashSet((Iterable)this.buttons);
