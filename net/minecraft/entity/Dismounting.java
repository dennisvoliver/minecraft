...
public class Dismounting {
   ...
   public static Vec3d method_30769(EntityType<?> entityType, CollisionView collisionView, BlockPos blockPos, boolean bl) {
      if (bl && entityType.isInvalidSpawn(collisionView.getBlockState(blockPos))) {
         ...
         double d = collisionView.getDismountHeight(getCollisionShape(collisionView, blockPos), () -> {
            return getCollisionShape(collisionView, blockPos.down());
         });
         if (!canDismountInBlock(d)) {
            return null;
         } else if (bl && d <= 0.0D && entityType.isInvalidSpawn(collisionView.getBlockState(blockPos.down()))) {
            return null;
         } else {
            Vec3d vec3d = Vec3d.ofCenter(blockPos, d);
            return collisionView.getBlockCollisions((Entity)null, entityType.getDimensions().getBoxAt(vec3d)).allMatch(VoxelShape::isEmpty) ? vec3d : null;
         }
      }
   }
}
