...
public class WalkTowardClosestAdultTask<E extends PassiveEntity> extends Task<E> {
   ...
   protected boolean shouldRun(ServerWorld serverWorld, E passiveEntity) {
      if (!passiveEntity.isBaby()) {
         ...
         return passiveEntity.isInRange(passiveEntity2, (double)(this.executionRange.getMax() + 1)) && !passiveEntity.isInRange(passiveEntity2, (double)this.executionRange.getMin());
      }
   }

   protected void run(ServerWorld serverWorld, E passiveEntity, long l) {
      LookTargetUtil.walkTowards(passiveEntity, (Entity)this.getNearestVisibleAdult(passiveEntity), this.speed, this.executionRange.getMin() - 1);
   }
