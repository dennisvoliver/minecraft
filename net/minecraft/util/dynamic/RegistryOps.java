...
public class RegistryOps<T> extends ForwardingDynamicOps<T> {
   ...
   protected <E> DataResult<Pair<Supplier<E>, T>> decodeOrId(T object, RegistryKey<? extends Registry<E>> registryKey, Codec<E> codec, boolean allowInlineDefinitions) {
      ...
      if (!optional.isPresent()) {
         ...
         if (!dataResult.result().isPresent()) {
            ...
            return this.readSupplier(registryKey, mutableRegistry, codec, identifier).map((supplier) -> {
      ...
      while(var7.hasNext()) {
         ...
         if (!string2.endsWith(".json")) {
            ...
            dataResult = dataResult.flatMap((simpleRegistry) -> {
               return this.readSupplier(registryKey, simpleRegistry, codec, identifier2).map((supplier) -> {
   ...
   private <E> DataResult<Supplier<E>> readSupplier(RegistryKey<? extends Registry<E>> registryKey, MutableRegistry<E> mutableRegistry, Codec<E> codec, Identifier elementId) {
      ...
      DataResult<Supplier<E>> dataResult = (DataResult)valueHolder.values.get(registryKey2);
      if (dataResult != null) {
         return dataResult;
      } else {
         ...
         if (!optional.isPresent() && mutableRegistry.get(registryKey2) != null) {
            dataResult4 = DataResult.success(() -> {
               return mutableRegistry.get(registryKey2);
            }, Lifecycle.stable());
         } else {
   ...
   protected <E> DataResult<Registry<E>> method_31152(RegistryKey<? extends Registry<E>> registryKey) {
      return (DataResult)this.registryManager.getOptional(registryKey).map((mutableRegistry) -> {
