...
public class ProfileResultImpl implements ProfileResult {
   ...
   private static final Comparator<Entry<String, ProfileResultImpl.CounterInfo>> COMPARATOR = Entry.comparingByValue(Comparator.comparingLong((counterInfo) -> {
   ...
   private Map<String, ProfileResultImpl.CounterInfo> setupCounters() {
      ...
      this.locationInfos.forEach((string, profileLocationInfo) -> {
         ...
         if (!object2LongMap.isEmpty()) {
            ...
            object2LongMap.forEach((stringx, long_) -> {
               ((ProfileResultImpl.CounterInfo)map.computeIfAbsent(stringx, (string) -> {
                  return new ProfileResultImpl.CounterInfo();
   ...
   public boolean save(File file) {
      ...
      try {
         ...
         LOGGER.error((String)"Could not save profiler results to {}", (Object)file, (Object)var8);
         var4 = false;
      } finally {
   ...
   private void appendTiming(int level, String name, StringBuilder sb) {
      ...
      Object2LongMap<String> object2LongMap = ((ProfileLocationInfo)ObjectUtils.firstNonNull((ProfileLocationInfo)this.locationInfos.get(name), EMPTY_INFO)).getCounts();
   ...
   private void appendCounter(int depth, String name, ProfileResultImpl.CounterInfo info, int tickSpan, StringBuilder sb) {
      ...
      info.subCounters.entrySet().stream().sorted(COMPARATOR).forEach((entry) -> {
         this.appendCounter(depth + 1, (String)entry.getKey(), (ProfileResultImpl.CounterInfo)entry.getValue(), tickSpan, sb);
      });
   }
   ...
   static class CounterInfo {
      ...
      public void add(Iterator<String> pathIterator, long time) {
         ...
         if (!pathIterator.hasNext()) {
            ...
            ((ProfileResultImpl.CounterInfo)this.subCounters.computeIfAbsent(pathIterator.next(), (string) -> {
               return new ProfileResultImpl.CounterInfo();
