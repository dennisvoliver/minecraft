...
public class EnderDragonEntityRenderer extends EntityRenderer<EnderDragonEntity> {
   ...
   public void render(EnderDragonEntity enderDragonEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
      ...
      if (enderDragonEntity.ticksSinceDeath > 0) {
         ...
         vertexConsumer4 = vertexConsumerProvider.getBuffer(DRAGON_CUTOUT);
         this.model.render(matrixStack, vertexConsumer4, i, OverlayTexture.getUv(0.0F, bl), 1.0F, 1.0F, 1.0F, 1.0F);
      }

      vertexConsumer4 = vertexConsumerProvider.getBuffer(DRAGON_EYES);
      ...
      if (enderDragonEntity.connectedCrystal != null) {
         ...
         renderCrystalBeam(l, m + EndCrystalEntityRenderer.getYOffset(enderDragonEntity.connectedCrystal, g), t, g, enderDragonEntity.age, matrixStack, vertexConsumerProvider, i);
         matrixStack.pop();
      }
   ...
   public static class DragonEntityModel extends EntityModel<EnderDragonEntity> {
      ...
      public void render(MatrixStack matrices, VertexConsumer vertices, int light, int overlay, float red, float green, float blue, float alpha) {
         ...
         this.head.yaw = MathHelper.fwrapDegrees(fs[0] - ds[0]) * 0.017453292F;
         this.head.pitch = MathHelper.fwrapDegrees((double)this.dragon.method_6823(6, ds, fs)) * 0.017453292F * 1.5F * 5.0F;
         this.head.roll = -MathHelper.fwrapDegrees(fs[0] - (double)m) * 0.017453292F;
         ...
         for(int s = 0; s < 12; ++s) {
            ...
            this.neck.yaw = (MathHelper.fwrapDegrees(fs[0] - ds[0]) * 1.5F + 180.0F) * 0.017453292F;
            this.neck.pitch = r + (float)(fs[1] - ds[1]) * 0.017453292F * 1.5F * 5.0F;
            this.neck.roll = MathHelper.fwrapDegrees(fs[0] - (double)m) * 0.017453292F * 1.5F;
            this.neck.pivotY = i;
            this.neck.pivotZ = j;
            this.neck.pivotX = h;
            i = (float)((double)i + Math.sin((double)this.neck.pitch) * 10.0D);
