...
public class SilverfishEntityModel<T extends Entity> extends CompositeEntityModel<T> {
   ...
   public SilverfishEntityModel() {
      ...
      for(int i = 0; i < this.body.length; ++i) {
         ...
         this.body[i].addCuboid((float)segmentLocations[i][0] * -0.5F, 0.0F, (float)segmentLocations[i][2] * -0.5F, (float)segmentLocations[i][0], (float)segmentLocations[i][1], (float)segmentLocations[i][2]);
      ...
      builder.addAll((Iterable)Arrays.asList(this.body));
      builder.addAll((Iterable)Arrays.asList(this.scales));
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
      for(int i = 0; i < this.body.length; ++i) {
         this.body[i].yaw = MathHelper.cos(animationProgress * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.05F * (float)(1 + Math.abs(i - 2));
         this.body[i].pivotX = MathHelper.sin(animationProgress * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.2F * (float)Math.abs(i - 2);
      }
