...
public class ChunkTaskPrioritySystem implements AutoCloseable, ChunkHolder.LevelUpdateListener {
   ...
   public ChunkTaskPrioritySystem(List<MessageListener<?>> actors, Executor executor, int maxQueues) {
      ...
      this.idleActors = Sets.newHashSet((Iterable)actors);
   ...
   public MessageListener<ChunkTaskPrioritySystem.UnblockingMessage> createUnblockingExecutor(MessageListener<Runnable> executor) {
      return (MessageListener)this.controlActor.ask((yield) -> {
         return new TaskQueue.PrioritizedTask(0, () -> {
            yield.send(MessageListener.create("chunk priority sorter around " + executor.getName(), (unblockingMessage) -> {
               this.removeChunk(executor, unblockingMessage.pos, unblockingMessage.callback, unblockingMessage.removeTask);
            }));
         });
      }).join();
   }
   ...
   private <T> void enqueueExecution(LevelPrioritizedQueue<Function<MessageListener<Unit>, T>> queue, MessageListener<T> actor) {
      this.controlActor.send(new TaskQueue.PrioritizedTask(3, () -> {
         ...
         if (stream == null) {
            ...
            Util.combine((List)stream.map((executeOrAddBlocking) -> {
   ...
   public String getDebugString() {
      return (String)this.queues.entrySet().stream().map((entry) -> {
         return ((MessageListener)entry.getKey()).getName() + "=[" + (String)((LevelPrioritizedQueue)entry.getValue()).getBlockingChunks().stream().map((long_) -> {
            return long_ + ":" + new ChunkPos(long_);
