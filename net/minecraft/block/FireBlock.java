...
public class FireBlock extends AbstractFireBlock {
   ...
   public FireBlock(AbstractBlock.Settings settings) {
      ...
      this.field_26658 = ImmutableMap.copyOf((Map)this.stateManager.getStates().stream().filter((blockState) -> {
         return (Integer)blockState.get(AGE) == 0;
   ...
   public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      return (VoxelShape)this.field_26658.get(state.with(AGE, 0));
   }
   ...
   private BlockState method_24855(WorldAccess worldAccess, BlockPos blockPos, int i) {
      ...
      return blockState.isOf(Blocks.FIRE) ? (BlockState)blockState.with(AGE, i) : blockState;
   }
   ...
   static {
      ...
      field_26653 = Block.createCuboidShape(0.0D, 15.0D, 0.0D, 16.0D, 16.0D, 16.0D);
      field_26654 = Block.createCuboidShape(0.0D, 0.0D, 0.0D, 1.0D, 16.0D, 16.0D);
      field_26655 = Block.createCuboidShape(15.0D, 0.0D, 0.0D, 16.0D, 16.0D, 16.0D);
      field_26656 = Block.createCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 16.0D, 1.0D);
      field_26657 = Block.createCuboidShape(0.0D, 0.0D, 15.0D, 16.0D, 16.0D, 16.0D);
   }
}
