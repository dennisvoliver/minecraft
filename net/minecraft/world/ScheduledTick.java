...
public class ScheduledTick<T> {
   ...
   public static <T> Comparator<ScheduledTick<T>> getComparator() {
      return Comparator.comparingLong((scheduledTick) -> {
         return scheduledTick.time;
      }).thenComparing((scheduledTick) -> {
         return scheduledTick.priority;
      }).thenComparingLong((scheduledTick) -> {
         return scheduledTick.id;
      });
   }
