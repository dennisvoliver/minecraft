...
public class OpenDoorsTask extends Task<LivingEntity> {
   ...
   public static void method_30760(ServerWorld serverWorld, LivingEntity livingEntity, @Nullable PathNode pathNode, @Nullable PathNode pathNode2) {
      ...
      if (brain.hasMemoryModule(MemoryModuleType.DOORS_TO_CLOSE)) {
         Iterator iterator = ((Set)brain.getOptionalMemory(MemoryModuleType.DOORS_TO_CLOSE).get()).iterator();
         ...
         while(true) {
            ...
            do {
               do {
                  ...
                  blockPos = globalPos.getPos();
               } while(pathNode != null && pathNode.getPos().equals(blockPos));
            } while(pathNode2 != null && pathNode2.getPos().equals(blockPos));
   ...
   private static boolean method_30761(ServerWorld serverWorld, LivingEntity livingEntity, BlockPos blockPos) {
      Brain<?> brain = livingEntity.getBrain();
      return !brain.hasMemoryModule(MemoryModuleType.MOBS) ? false : ((List)brain.getOptionalMemory(MemoryModuleType.MOBS).get()).stream().filter((livingEntity2) -> {
         return livingEntity2.getType() == livingEntity.getType();
      }).filter((livingEntityx) -> {
         return blockPos.isWithinDistance(livingEntityx.getPos(), 2.0D);
      }).anyMatch((livingEntityx) -> {
         return method_30766(serverWorld, livingEntityx, blockPos);
      });
   }
   ...
   private static boolean method_30762(ServerWorld serverWorld, LivingEntity livingEntity, GlobalPos globalPos) {
      return globalPos.getDimension() != serverWorld.getRegistryKey() || !globalPos.getPos().isWithinDistance(livingEntity.getPos(), 2.0D);
   }
   ...
   private void method_30767(ServerWorld serverWorld, LivingEntity livingEntity, BlockPos blockPos) {
      ...
      if (brain.getOptionalMemory(MemoryModuleType.DOORS_TO_CLOSE).isPresent()) {
         ...
         brain.remember(MemoryModuleType.DOORS_TO_CLOSE, (Object)Sets.newHashSet((Object[])(globalPos)));
      }
