...
public class ShipwreckGenerator {
   ...
   public static void addParts(StructureManager structureManager, BlockPos pos, BlockRotation rotation, List<StructurePiece> children, Random random, ShipwreckFeatureConfig config) {
      Identifier identifier = (Identifier)Util.getRandom((Object[])(config.isBeached ? BEACHED_TEMPLATES : REGULAR_TEMPLATES), random);
   ...
   public static class Piece extends SimpleStructurePiece {
      ...
      private void initializeStructureData(StructureManager manager) {
         ...
         StructurePlacementData structurePlacementData = (new StructurePlacementData()).setRotation(this.rotation).setMirror(BlockMirror.NONE).setPosition(ShipwreckGenerator.field_14536).addProcessor(BlockIgnoreStructureProcessor.IGNORE_AIR_AND_STRUCTURE_BLOCKS);
      ...
      public boolean generate(StructureWorldAccess structureWorldAccess, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox boundingBox, ChunkPos chunkPos, BlockPos blockPos) {
         ...
         if (k == 0) {
            ...
            int l;
            for(Iterator var14 = BlockPos.iterate(this.pos, blockPos3).iterator(); var14.hasNext(); i = Math.min(i, l)) {
               BlockPos blockPos4 = (BlockPos)var14.next();
               l = structureWorldAccess.getTopY(type, blockPos4.getX(), blockPos4.getZ());
               j += l;
            }
