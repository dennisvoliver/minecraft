...
public enum DirectionTransformation implements StringIdentifiable {
   ...
   private static final DirectionTransformation[][] COMBINATIONS = (DirectionTransformation[][])Util.make(new DirectionTransformation[values().length][values().length], (directionTransformations) -> {
      Map<Pair<AxisTransformation, BooleanList>, DirectionTransformation> map = (Map)Arrays.stream(values()).collect(Collectors.toMap((directionTransformationx) -> {
         return Pair.of(directionTransformationx.axisTransformation, directionTransformationx.getAxisFlips());
      }, (directionTransformationx) -> {
         return directionTransformationx;
      }));
      ...
      for(int var4 = 0; var4 < var3; ++var4) {
         ...
         for(int var8 = 0; var8 < var7; ++var8) {
            ...
            directionTransformations[directionTransformation.ordinal()][directionTransformation2.ordinal()] = (DirectionTransformation)map.get(Pair.of(axisTransformation, booleanArrayList));
         }
      }

   });
   private static final DirectionTransformation[] INVERSES = (DirectionTransformation[])Arrays.stream(values()).map((directionTransformation) -> {
      return (DirectionTransformation)Arrays.stream(values()).filter((directionTransformation2) -> {
         return directionTransformation.prepend(directionTransformation2) == IDENTITY;
      }).findAny().get();
   }).toArray((i) -> {
