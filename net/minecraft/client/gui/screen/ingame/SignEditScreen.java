...
public class SignEditScreen extends Screen {
   ...
   public SignEditScreen(SignBlockEntity sign) {
      ...
      sign.getClass();
      this.text = (String[])var10001.mapToObj(sign::getTextOnRow).map(Text::getString).toArray((i) -> {
         return new String[i];
      });
      this.sign = sign;
   }
   ...
   protected void init() {
      ...
      this.addButton(new ButtonWidget(this.width / 2 - 100, this.height / 4 + 120, 200, 20, ScreenTexts.DONE, (buttonWidget) -> {
         this.finishEditing();
      }));
