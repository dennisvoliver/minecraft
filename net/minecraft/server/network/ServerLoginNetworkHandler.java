...
public class ServerLoginNetworkHandler implements ServerLoginPacketListener {
   ...
   public void disconnect(Text reason) {
      try {
         LOGGER.info((String)"Disconnecting {}: {}", (Object)this.getConnectionInfo(), (Object)reason.getString());
   ...
   public void onDisconnected(Text reason) {
      LOGGER.info((String)"{} lost connection: {}", (Object)this.getConnectionInfo(), (Object)reason.getString());
   }
   ...
   public void onKey(LoginKeyC2SPacket packet) {
      ...
      try {
         ...
         string2 = (new BigInteger(NetworkEncryptionUtils.generateServerId("", this.server.getKeyPair().getPublic(), this.secretKey))).toString(16);
      ...
      Thread thread = new Thread("User Authenticator #" + authenticatorThreadId.incrementAndGet()) {
         public void run() {
            ...
            try {
               ServerLoginNetworkHandler.this.profile = ServerLoginNetworkHandler.this.server.getSessionService().hasJoinedServer(new GameProfile((UUID)null, gameProfile.getName()), string2, this.getClientAddress());
               if (ServerLoginNetworkHandler.this.profile != null) {
                  ServerLoginNetworkHandler.LOGGER.info((String)"UUID of player {} is {}", (Object)ServerLoginNetworkHandler.this.profile.getName(), (Object)ServerLoginNetworkHandler.this.profile.getId());
                  ...
                  ServerLoginNetworkHandler.LOGGER.error((String)"Username '{}' tried to join with an invalid session", (Object)gameProfile.getName());
               }
