...
public class HoverEvent {
      ...
      if (string == null) {
         ...
         if (action == null) {
            ...
            if (jsonElement != null) {
               ...
               return text != null ? action.buildHoverEvent((Text)text) : null;
            }
         }
      }
   }
   ...
   public static class Action<T> {
      public static final HoverEvent.Action<Text> SHOW_TEXT = new HoverEvent.Action("show_text", true, Text.Serializer::fromJson, Text.Serializer::toJsonTree, Function.identity());
      public static final HoverEvent.Action<HoverEvent.ItemStackContent> SHOW_ITEM = new HoverEvent.Action("show_item", true, (jsonElement) -> {
         return HoverEvent.ItemStackContent.parse(jsonElement);
      }, (object) -> {
         return ((HoverEvent.ItemStackContent)object).toJson();
      }, (text) -> {
         return HoverEvent.ItemStackContent.parse(text);
      });
      public static final HoverEvent.Action<HoverEvent.EntityContent> SHOW_ENTITY = new HoverEvent.Action("show_entity", true, HoverEvent.EntityContent::parse, HoverEvent.EntityContent::toJson, HoverEvent.EntityContent::parse);
      ...
      static {
         BY_NAME = (Map)Stream.of(SHOW_TEXT, SHOW_ITEM, SHOW_ENTITY).collect(ImmutableMap.toImmutableMap(HoverEvent.Action::getName, (action) -> {
            return action;
         }));
      }
   }
   ...
   public static class ItemStackContent {
      ...
      private static HoverEvent.ItemStackContent parse(JsonElement json) {
         if (json.isJsonPrimitive()) {
            return new HoverEvent.ItemStackContent((Item)Registry.ITEM.get(new Identifier(json.getAsString())), 1, (CompoundTag)null);
         } else {
            JsonObject jsonObject = JsonHelper.asObject(json, "item");
            Item item = (Item)Registry.ITEM.get(new Identifier(JsonHelper.getString(jsonObject, "id")));
            ...
            if (jsonObject.has("tag")) {
               ...
               try {
                  ...
                  HoverEvent.LOGGER.warn((String)"Failed to parse tag: {}", (Object)string, (Object)var6);
               }
            }

            return new HoverEvent.ItemStackContent(item, i, (CompoundTag)null);
         }
      }
      ...
      private static HoverEvent.ItemStackContent parse(Text text) {
         try {
            ...
            HoverEvent.LOGGER.warn((String)"Failed to parse item tag: {}", (Object)text, (Object)var2);
            return null;
         }
      }
   ...
   public static class EntityContent {
      ...
      public static HoverEvent.EntityContent parse(JsonElement json) {
         if (!json.isJsonObject()) {
            ...
            EntityType<?> entityType = (EntityType)Registry.ENTITY_TYPE.get(new Identifier(JsonHelper.getString(jsonObject, "type")));
      ...
      public static HoverEvent.EntityContent parse(Text text) {
         try {
            ...
            EntityType<?> entityType = (EntityType)Registry.ENTITY_TYPE.get(new Identifier(compoundTag.getString("type")));
      ...
      public List<Text> asTooltip() {
         if (this.tooltip == null) {
            ...
            this.tooltip.add(new TranslatableText("gui.entity_tooltip.type", new Object[]{this.entityType.getName()}));
