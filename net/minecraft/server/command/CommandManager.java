...
public class CommandManager {
   ...
   public CommandManager(CommandManager.RegistrationEnvironment environment) {
      ...
      this.dispatcher.findAmbiguities((commandNode, commandNode2, commandNode3, collection) -> {
         LOGGER.warn((String)"Ambiguity between arguments {} and {} with inputs: {}", (Object)this.dispatcher.getPath(commandNode2), this.dispatcher.getPath(commandNode3), collection);
      });
   ...
   public int execute(ServerCommandSource commandSource, String command) {
      ...
      try {
         byte var18;
         try {
            int var4 = this.dispatcher.execute((StringReader)stringReader, commandSource);
            return var4;
            ...
            if (var14.getInput() != null && var14.getCursor() >= 0) {
               ...
               mutableText.append((Text)(new TranslatableText("command.context.here")).formatted(new Formatting[]{Formatting.RED, Formatting.ITALIC}));
            ...
            MutableText mutableText2 = var10000;
            if (LOGGER.isDebugEnabled()) {
               LOGGER.error((String)"Command exception: {}", (Object)command, (Object)var15);
            ...
            commandSource.sendError((new TranslatableText("command.failed")).styled((style) -> {
               return style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, mutableText2));
            }));
            if (SharedConstants.isDevelopment) {
               commandSource.sendError(new LiteralText(Util.getInnermostMessage(var15)));
               LOGGER.error((String)("'" + command + "' threw an exception"), (Throwable)var15);
            }
   ...
   public static void checkMissing() {
      ...
      Set<ArgumentType<?>> set2 = (Set)set.stream().filter((argumentType) -> {
         return !ArgumentTypes.hasClass(argumentType);
      }).collect(Collectors.toSet());
      if (!set2.isEmpty()) {
         LOGGER.warn("Missing type registration for following arguments:\n {}", set2.stream().map((argumentType) -> {
            return "\t" + argumentType;
