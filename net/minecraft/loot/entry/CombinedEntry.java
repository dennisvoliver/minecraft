...
public abstract class CombinedEntry extends LootPoolEntry {
   ...
   public static <T extends CombinedEntry> LootPoolEntry.Serializer<T> createSerializer(final CombinedEntry.Factory<T> factory) {
      return new LootPoolEntry.Serializer<T>() {
         ...
         public final T fromJson(JsonObject jsonObject, JsonDeserializationContext jsonDeserializationContext, LootCondition[] lootConditions) {
            LootPoolEntry[] lootPoolEntrys = (LootPoolEntry[])JsonHelper.deserialize(jsonObject, "children", jsonDeserializationContext, LootPoolEntry[].class);
