...
public class GameRules {
   ...
   private void load(DynamicLike<?> dynamicLike) {
      this.rules.forEach((key, rule) -> {
         dynamicLike.get(key.name).asString().result().ifPresent(rule::deserialize);
      });
   }
   ...
   public static class Type<T extends GameRules.Rule<T>> {
      ...
      private Type(Supplier<ArgumentType<?>> argumentType, Function<GameRules.Type<T>, T> ruleFactory, BiConsumer<MinecraftServer, T> changeCallback, GameRules.Acceptor<T> ruleAcceptor) {
