...
public class RealmsServer extends ValueObject {
   ...
   public void updateServerPing(RealmsServerPlayerList serverPlayerList) {
      ...
      while(true) {
         ...
         try {
            ...
            LOGGER.error((String)("Could not get name for " + string), (Throwable)var8);
            continue;
         }
   ...
   private static void sortInvited(RealmsServer server) {
      server.players.sort((playerInfo, playerInfo2) -> {
         return ComparisonChain.start().compareFalseFirst(playerInfo2.getAccepted(), playerInfo.getAccepted()).compare((Comparable)playerInfo.getName().toLowerCase(Locale.ROOT), (Comparable)playerInfo2.getName().toLowerCase(Locale.ROOT)).result();
      });
   }
