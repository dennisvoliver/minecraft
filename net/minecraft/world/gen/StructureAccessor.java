...
public class StructureAccessor {
   ...
   public Stream<? extends StructureStart<?>> getStructuresWithChildren(ChunkSectionPos pos, StructureFeature<?> feature) {
      return this.world.getChunk(pos.getSectionX(), pos.getSectionZ(), ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences(feature).stream().map((posx) -> {
         return ChunkSectionPos.from(new ChunkPos(posx), 0);
      }).map((posx) -> {
         return this.getStructureStart(posx, feature, this.world.getChunk(posx.getSectionX(), posx.getSectionZ(), ChunkStatus.STRUCTURE_STARTS));
   ...
   public StructureStart<?> getStructureAt(BlockPos pos, boolean matchChildren, StructureFeature<?> feature) {
      return (StructureStart)DataFixUtils.orElse(this.getStructuresWithChildren(ChunkSectionPos.from(pos), feature).filter((structureStart) -> {
         ...
         return !matchChildren || structureStart.getChildren().stream().anyMatch((piece) -> {
            return piece.getBoundingBox().contains(pos);
         });
