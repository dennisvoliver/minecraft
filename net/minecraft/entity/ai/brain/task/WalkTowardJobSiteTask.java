...
public class WalkTowardJobSiteTask extends Task<VillagerEntity> {
   ...
   protected void keepRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      LookTargetUtil.walkTowards(villagerEntity, (BlockPos)((GlobalPos)villagerEntity.getBrain().getOptionalMemory(MemoryModuleType.POTENTIAL_JOB_SITE).get()).getPos(), this.speed, 1);
   }
   ...
   protected void finishRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      ...
      optional.ifPresent((globalPos) -> {
         ...
         if (serverWorld2 != null) {
            ...
            if (pointOfInterestStorage.test(blockPos, (pointOfInterestType) -> {
               return true;
            })) {
