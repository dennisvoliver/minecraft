...
public class ForgetCompletedPointOfInterestTask extends Task<LivingEntity> {
   ...
   protected boolean shouldRun(ServerWorld world, LivingEntity entity) {
      ...
      return world.getRegistryKey() == globalPos.getDimension() && globalPos.getPos().isWithinDistance(entity.getPos(), 16.0D);
   }
   ...
   protected void run(ServerWorld world, LivingEntity entity, long time) {
      ...
      if (serverWorld != null && !this.hasCompletedPointOfInterest(serverWorld, blockPos)) {
         if (this.isBedOccupiedByOthers(serverWorld, blockPos, entity)) {
