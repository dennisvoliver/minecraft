...
public class LevelStorage {
   ...
   private static DataPackSettings readDataPackSettings(File file, DataFixer dataFixer) {
      try {
         ...
         Dynamic<Tag> dynamic = dataFixer.update(DataFixTypes.LEVEL.getTypeReference(), new Dynamic(NbtOps.INSTANCE, compoundTag2), i, SharedConstants.getGameVersion().getWorldVersion());
   ...
   private static BiFunction<File, DataFixer, LevelProperties> createLevelDataParser(DynamicOps<Tag> dynamicOps, DataPackSettings dataPackSettings) {
      return (file, dataFixer) -> {
         try {
            ...
            compoundTag2.remove("Player");
            int i = compoundTag2.contains("DataVersion", 99) ? compoundTag2.getInt("DataVersion") : -1;
            Dynamic<Tag> dynamic = dataFixer.update(DataFixTypes.LEVEL.getTypeReference(), new Dynamic(dynamicOps, compoundTag2), i, SharedConstants.getGameVersion().getWorldVersion());
            ...
            return LevelProperties.readProperties(dynamic, dataFixer, i, compoundTag3, levelInfo, saveVersionInfo, (GeneratorOptions)pair.getFirst(), (Lifecycle)pair.getSecond());
   ...
   public class Session implements AutoCloseable {
      ...
      public DataPackSettings getDataPackSettings() {
         this.checkValid();
         return (DataPackSettings)LevelStorage.this.readLevelProperties(this.directory.toFile(), (file, dataFixer) -> {
            return LevelStorage.readDataPackSettings(file, dataFixer);
         });
      }

      public void backupLevelDataFile(DynamicRegistryManager dynamicRegistryManager, SaveProperties saveProperties) {
         this.backupLevelDataFile(dynamicRegistryManager, saveProperties, (CompoundTag)null);
      }
