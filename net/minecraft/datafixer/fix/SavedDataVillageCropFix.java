...
public class SavedDataVillageCropFix extends DataFix {
   ...
   private static <T> Dynamic<T> method_5157(Dynamic<T> dynamic) {
      DataResult var10000 = dynamic.asStreamOpt().map(SavedDataVillageCropFix::fixVillageChildren);
      dynamic.getClass();
   ...
   private static <T> Dynamic<T> fixCropId(Dynamic<T> dynamic, String cropId) {
      return dynamic.get(cropId).asNumber().result().isPresent() ? dynamic.set(cropId, BlockStateFlattening.lookupState(dynamic.get(cropId).asInt(0) << 4)) : dynamic;
   }
}
