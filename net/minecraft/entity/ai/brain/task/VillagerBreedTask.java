...
public class VillagerBreedTask extends Task<VillagerEntity> {
   ...
   private boolean isReadyToBreed(VillagerEntity villager) {
      ...
      Optional<PassiveEntity> optional = brain.getOptionalMemory(MemoryModuleType.BREED_TARGET).filter((passiveEntity) -> {
         return passiveEntity.getType() == EntityType.VILLAGER;
      });
      if (!optional.isPresent()) {
         return false;
      } else {
         return LookTargetUtil.canSee(brain, MemoryModuleType.BREED_TARGET, EntityType.VILLAGER) && villager.isReadyToBreed() && ((PassiveEntity)optional.get()).isReadyToBreed();
      }
   }
