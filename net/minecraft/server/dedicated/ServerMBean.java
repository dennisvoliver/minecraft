...
public final class ServerMBean implements DynamicMBean {
   ...
   private ServerMBean(MinecraftServer server) {
      ...
      this.server = server;
      MBeanAttributeInfo[] mBeanAttributeInfos = (MBeanAttributeInfo[])this.entries.values().stream().map((object) -> {
         return ((ServerMBean.Entry)object).createInfo();
      }).toArray((i) -> {
         return new MBeanAttributeInfo[i];
      });
      this.beanInfo = new MBeanInfo(ServerMBean.class.getSimpleName(), "metrics for dedicated server", mBeanAttributeInfos, (MBeanConstructorInfo[])null, (MBeanOperationInfo[])null, new MBeanNotificationInfo[0]);
   }
