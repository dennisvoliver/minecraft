...
public class HandledScreens {
   ...
   public static <T extends ScreenHandler> void open(@Nullable ScreenHandlerType<T> type, MinecraftClient client, int id, Text title) {
      if (type == null) {
         LOGGER.warn((String)"Trying to open invalid screen with name: {}", (Object)title.getString());
      } else {
         HandledScreens.Provider<T, ?> provider = getProvider(type);
         if (provider == null) {
            LOGGER.warn((String)"Failed to create screen for menu type: {}", (Object)Registry.SCREEN_HANDLER.getId(type));
         } else {
   ...
   public static boolean validateScreens() {
      ...
      while(var1.hasNext()) {
         ...
         if (!PROVIDERS.containsKey(screenHandlerType)) {
            LOGGER.debug((String)"Menu {} has no matching screen", (Object)Registry.SCREEN_HANDLER.getId(screenHandlerType));
            bl = true;
         }
      }
