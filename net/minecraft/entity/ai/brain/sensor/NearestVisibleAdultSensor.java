...
public class NearestVisibleAdultSensor extends Sensor<PassiveEntity> {
   ...
   protected void sense(ServerWorld serverWorld, PassiveEntity passiveEntity) {
      passiveEntity.getBrain().getOptionalMemory(MemoryModuleType.VISIBLE_MOBS).ifPresent((list) -> {
         this.findNearestVisibleAdult(passiveEntity, list);
      });
   }

   private void findNearestVisibleAdult(PassiveEntity entity, List<LivingEntity> visibleMobs) {
      Optional<PassiveEntity> optional = visibleMobs.stream().filter((livingEntity) -> {
         return livingEntity.getType() == entity.getType();
      }).map((livingEntity) -> {
         return (PassiveEntity)livingEntity;
      }).filter((passiveEntity) -> {
         return !passiveEntity.isBaby();
      }).findFirst();
