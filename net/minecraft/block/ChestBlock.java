...
public class ChestBlock extends AbstractChestBlock<ChestBlockEntity> implements Waterloggable {
   ...
   public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
      ...
      if (newState.isOf(this) && direction.getAxis().isHorizontal()) {
         ChestType chestType = (ChestType)newState.get(CHEST_TYPE);
         if (state.get(CHEST_TYPE) == ChestType.SINGLE && chestType != ChestType.SINGLE && state.get(FACING) == newState.get(FACING) && getFacing(newState) == direction.getOpposite()) {
