...
public class SocialInteractionsScreen extends Screen {
   ...
   protected void init() {
      ...
      this.allTabButton = (ButtonWidget)this.addButton(new ButtonWidget(j, 45, i, 20, ALL_TAB_TITLE, (buttonWidget) -> {
         this.setCurrentTab(SocialInteractionsScreen.Tab.ALL);
      }));
      this.hiddenTabButton = (ButtonWidget)this.addButton(new ButtonWidget((j + k - i) / 2 + 1, 45, i, 20, HIDDEN_TAB_TITLE, (buttonWidget) -> {
         this.setCurrentTab(SocialInteractionsScreen.Tab.HIDDEN);
      }));
      this.blockedTabButton = (ButtonWidget)this.addButton(new ButtonWidget(k - i + 1, 45, i, 20, BLOCKED_TAB_TITLE, (buttonWidget) -> {
         this.setCurrentTab(SocialInteractionsScreen.Tab.BLOCKED);
      }));
      this.blockingButton = (ButtonWidget)this.addButton(new ButtonWidget(n, m, l, 20, BLOCKING_TEXT, (buttonWidget) -> {
   ...
   private void setCurrentTab(SocialInteractionsScreen.Tab currentTab) {
      ...
      case ALL:
         ...
         collection4 = this.client.player.networkHandler.getPlayerUuids();
         break;
      case HIDDEN:
         this.hiddenTabButton.setMessage(SELECTED_HIDDEN_TAB_TITLE);
         collection4 = this.client.getSocialInteractionsManager().getHiddenPlayers();
         break;
      case BLOCKED:
         ...
         socialInteractionsManager.getClass();
         collection4 = (Collection)var10000.filter(socialInteractionsManager::isPlayerBlocked).collect(Collectors.toSet());
         break;
      default:
         collection4 = ImmutableList.of();
      }

      this.currentTab = currentTab;
      this.playerList.update((Collection)collection4, this.playerList.getScrollAmount());
      ...
      } else if (((Collection)collection4).isEmpty()) {
   ...
   private void updateServerLabel(MinecraftClient client) {
      ...
      if (this.playerCount != i) {
         ...
         if (i > 1) {
            this.serverLabel = new TranslatableText("gui.socialInteractions.server_label.multiple", new Object[]{string, i});
         } else {
            this.serverLabel = new TranslatableText("gui.socialInteractions.server_label.single", new Object[]{string, i});
         }
   ...
   static {
      SELECTED_ALL_TAB_TITLE = ALL_TAB_TITLE.copy().formatted(Formatting.UNDERLINE);
      SELECTED_HIDDEN_TAB_TITLE = HIDDEN_TAB_TITLE.copy().formatted(Formatting.UNDERLINE);
      SELECTED_BLOCKED_TAB_TITLE = BLOCKED_TAB_TITLE.copy().formatted(Formatting.UNDERLINE);
      SEARCH_TEXT = (new TranslatableText("gui.socialInteractions.search_hint")).formatted(Formatting.ITALIC).formatted(Formatting.GRAY);
      EMPTY_SEARCH_TEXT = (new TranslatableText("gui.socialInteractions.search_empty")).formatted(Formatting.GRAY);
      EMPTY_HIDDEN_TEXT = (new TranslatableText("gui.socialInteractions.empty_hidden")).formatted(Formatting.GRAY);
      EMPTY_BLOCKED_TEXT = (new TranslatableText("gui.socialInteractions.empty_blocked")).formatted(Formatting.GRAY);
      BLOCKING_TEXT = new TranslatableText("gui.socialInteractions.blocking_hint");
   }
