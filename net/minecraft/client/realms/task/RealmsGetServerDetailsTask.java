   ...
   public void run() {
      ...
      for(int j = 0; j < 40 && !this.aborted(); ++j) {
         try {
            ...
            bl2 = true;
            LOGGER.error((String)"Couldn't connect to world", (Throwable)var13);
      ...
      if (bl3) {
         ...
         if (this.server.ownerUUID.equals(MinecraftClient.getInstance().getSession().getUuid())) {
            ...
            setScreen(new RealmsGenericErrorScreen(new TranslatableText("mco.brokenworld.nonowner.title"), new TranslatableText("mco.brokenworld.nonowner.error"), this.lastScreen));
         }
         ...
         if (bl) {
            if (realmsServerAddress.resourcePackUrl != null && realmsServerAddress.resourcePackHash != null) {
               ...
               setScreen(new RealmsLongConfirmationScreen((blx) -> {
                  try {
                     if (blx) {
                        Function function = (throwable) -> {
                        ...
                        try {
                           MinecraftClient.getInstance().getResourcePackDownloader().download(realmsServerAddress.resourcePackUrl, realmsServerAddress.resourcePackHash).thenRun(() -> {
