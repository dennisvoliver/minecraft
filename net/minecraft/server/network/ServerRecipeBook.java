      ...
      for(int i = 0; i < list.size(); ++i) {
         ...
         try {
            ...
            if (!optional.isPresent()) {
               LOGGER.error((String)"Tried to load unrecognized recipe: {} removed now.", (Object)identifier);
            } else {
               handler.accept(optional.get());
            }
         } catch (InvalidIdentifierException var8) {
            LOGGER.error((String)"Tried to load improperly formatted recipe: {} removed now.", (Object)string);
         }
      }
