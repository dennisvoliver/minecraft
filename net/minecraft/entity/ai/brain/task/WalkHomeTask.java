...
public class WalkHomeTask extends Task<LivingEntity> {
   ...
   protected boolean shouldRun(ServerWorld world, LivingEntity entity) {
      if (world.getTime() - this.expiryTimeLimit < 20L) {
         ...
         return optional.isPresent() && !(((BlockPos)optional.get()).getSquaredDistance(pathAwareEntity.getBlockPos()) <= 4.0D);
      }
   }
   ...
   protected void run(ServerWorld world, LivingEntity entity, long time) {
      ...
      if (path != null && path.reachesTarget()) {
         ...
         if (optional.isPresent()) {
            entity.getBrain().remember(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget(blockPos, this.speed, 1)));
