...
public class ScheduleBuilder {
   ...
   public Schedule build() {
      Set var10000 = (Set)this.activities.stream().map(ScheduleBuilder.ActivityEntry::getActivity).collect(Collectors.toSet());
      ...
      this.activities.forEach((activityEntry) -> {
         ...
         this.schedule.getOtherRules(activity).forEach((scheduleRule) -> {
            scheduleRule.add(activityEntry.getStartTime(), 0.0F);
         });
