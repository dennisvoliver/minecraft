...
public final class VillagerType {
   ...
   private static final Map<RegistryKey<Biome>, VillagerType> BIOME_TO_TYPE = (Map)Util.make(Maps.newHashMap(), (hashMap) -> {
   ...
   private static VillagerType create(String id) {
      return (VillagerType)Registry.register(Registry.VILLAGER_TYPE, (Identifier)(new Identifier(id)), new VillagerType(id));
   }

   public static VillagerType forBiome(Optional<RegistryKey<Biome>> optional) {
      return (VillagerType)optional.flatMap((registryKey) -> {
         return Optional.ofNullable(BIOME_TO_TYPE.get(registryKey));
      }).orElse(PLAINS);
   }
}
