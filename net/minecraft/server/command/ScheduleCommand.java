   ...
   private static int execute(ServerCommandSource source, Pair<Identifier, Either<CommandFunction, Tag<CommandFunction>>> pair, int i, boolean bl) throws CommandSyntaxException {
      if (i == 0) {
         ...
         ((Either)pair.getSecond()).ifLeft((commandFunction) -> {
            ...
            source.sendFeedback(new TranslatableText("commands.schedule.created.function", new Object[]{identifier, i, l}), true);
         }).ifRight((tag) -> {
            String string = "#" + identifier.toString();
            if (bl) {
               timer.method_22593(string);
            ...
            source.sendFeedback(new TranslatableText("commands.schedule.created.tag", new Object[]{identifier, i, l}), true);
         });
