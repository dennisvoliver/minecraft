...
public class TextRenderer {
   ...
   private int draw(OrderedText text, float x, float y, int color, Matrix4f matrix, boolean shadow) {
      VertexConsumerProvider.Immediate immediate = VertexConsumerProvider.immediate(Tessellator.getInstance().getBuffer());
      int i = this.draw((OrderedText)text, x, y, color, shadow, matrix, immediate, false, 0, 15728880);
      immediate.draw();
      return i;
   }
   ...
   private float drawLayer(String text, float x, float y, int color, boolean shadow, Matrix4f matrix, VertexConsumerProvider vertexConsumerProvider, boolean seeThrough, int underlineColor, int light) {
      ...
      TextVisitFactory.visitFormatted((String)text, Style.EMPTY, drawer);
   ...
   class Drawer implements CharacterVisitor {
      ...
      public boolean accept(int i, Style style, int j) {
         ...
         if (!(glyphRenderer instanceof EmptyGlyphRenderer)) {
            ...
            TextRenderer.this.drawGlyph(glyphRenderer, bl, style.isItalic(), r, this.x + s, this.y + s, this.matrix, vertexConsumer, m, n, o, f, this.light);
         }
