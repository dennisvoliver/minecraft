...
public class ServerWorld extends World implements StructureWorldAccess {
   ...
   public ServerWorld(MinecraftServer server, Executor workerExecutor, LevelStorage.Session session, ServerWorldProperties properties, RegistryKey<World> registryKey, DimensionType dimensionType, WorldGenerationProgressListener worldGenerationProgressListener, ChunkGenerator chunkGenerator, boolean debugWorld, long l, List<Spawner> list, boolean bl) {
      ...
      this.serverChunkManager = new ServerChunkManager(this, session, server.getDataFixer(), server.getStructureManager(), workerExecutor, chunkGenerator, server.getPlayerManager().getViewDistance(), server.syncChunkWrites(), worldGenerationProgressListener, () -> {
   ...
   public void tick(BooleanSupplier shouldKeepTicking) {
      ...
      if (this.allPlayersSleeping && this.players.stream().noneMatch((player) -> {
         return !player.isSpectator() && !player.isSleepingLongEnough();
      })) {
   ...
   public void tickPassenger(Entity vehicle, Entity passenger) {
      if (!passenger.removed && passenger.getVehicle() == vehicle) {
         if (passenger instanceof PlayerEntity || this.getChunkManager().shouldTickEntity(passenger)) {
            ...
            if (passenger.updateNeeded) {
               Iterator var5 = passenger.getPassengerList().iterator();
   ...
   private boolean checkUuid(Entity entity) {
      ...
      if (entity2 == null) {
         ...
         LOGGER.warn((String)"Trying to add entity with duplicated UUID {}. Existing {}#{}, new: {}#{}", (Object)uUID, EntityType.getId(entity2.getType()), entity2.getEntityId(), EntityType.getId(entity.getType()), entity.getEntityId());
         return true;
      }
   }
   ...
   private Entity checkIfUuidExists(UUID uUID) {
      ...
      if (entity != null) {
         ...
         if (this.inEntityTick) {
            Iterator var3 = this.entitiesToLoad.iterator();
   ...
   public Explosion createExplosion(@Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior explosionBehavior, double d, double e, double f, float g, boolean bl, Explosion.DestructionType destructionType) {
      ...
      while(var14.hasNext()) {
         ...
         if (serverPlayerEntity.squaredDistanceTo(d, e, f) < 4096.0D) {
            serverPlayerEntity.networkHandler.sendPacket(new ExplosionS2CPacket(d, e, f, g, explosion.getAffectedBlocks(), (Vec3d)explosion.getAffectedPlayers().get(serverPlayerEntity)));
         }
      }
   ...
   public int getNextMapId() {
      return ((IdCountsState)this.getServer().getOverworld().getPersistentStateManager().getOrCreate(IdCountsState::new, "idcounts")).getNextMapId();
   }
   ...
   public void dump(Path path) throws IOException {
      ...
      try {
         ...
         if (info != null) {
            ObjectIterator var6 = info.getGroupToCount().object2IntEntrySet().iterator();

            while(var6.hasNext()) {
               it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<SpawnGroup> entry = (it.unimi.dsi.fastutil.objects.Object2IntMap.Entry)var6.next();
   ...
   public String method_31268() {
      return String.format("players: %s, entities: %d [%s], block_entities: %d [%s], block_ticks: %d, fluid_ticks: %d, chunk_source: %s", this.players.size(), this.entitiesById.size(), method_31270(this.entitiesById.values(), (entity) -> {
         return Registry.ENTITY_TYPE.getId(entity.getType());
      }), this.tickingBlockEntities.size(), method_31270(this.tickingBlockEntities, (blockEntity) -> {
         return Registry.BLOCK_ENTITY_TYPE.getId(blockEntity.getType());
      }), this.getBlockTickScheduler().getTicks(), this.getFluidTickScheduler().getTicks(), this.getDebugString());
   }
   ...
   private static <T> String method_31270(Collection<T> collection, Function<T, Identifier> function) {
      try {
         ...
         return (String)object2IntOpenHashMap.object2IntEntrySet().stream().sorted(Comparator.comparing(it.unimi.dsi.fastutil.objects.Object2IntMap.Entry::getIntValue).reversed()).limit(5L).map((entry) -> {
