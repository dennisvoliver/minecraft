...
public class Source {
      ...
      if (this.playing.compareAndSet(true, false)) {
         ...
         if (this.stream != null) {
            try {
               ...
               LOGGER.error((String)"Failed to close audio stream", (Throwable)var2);
            }
   ...
   public void setPosition(Vec3d vec3d) {
      AL10.alSourcefv(this.pointer, 4100, (float[])(new float[]{(float)vec3d.x, (float)vec3d.y, (float)vec3d.z}));
   }
   ...
   private void method_19640(int i) {
      if (this.stream != null) {
         try {
            ...
            LOGGER.error((String)"Failed to read from audio stream", (Throwable)var4);
         }
      }
