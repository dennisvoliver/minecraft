...
public class ParrotEntity extends TameableShoulderEntity implements Flutterer {
   ...
   public static boolean imitateNearbyMob(World world, Entity parrot) {
      if (parrot.isAlive() && !parrot.isSilent() && world.random.nextInt(2) == 0) {
         List<MobEntity> list = world.getEntitiesByClass(MobEntity.class, parrot.getBoundingBox().expand(20.0D), CAN_IMITATE);
         if (!list.isEmpty()) {
   ...
   public static SoundEvent getRandomSound(World world, Random random) {
      if (world.getDifficulty() != Difficulty.PEACEFUL && random.nextInt(1000) == 0) {
         List<EntityType<?>> list = Lists.newArrayList((Iterable)MOB_SOUNDS.keySet());
   ...
   static {
      ...
      COOKIE = Items.COOKIE;
      TAMING_INGREDIENTS = Sets.newHashSet((Object[])(Items.WHEAT_SEEDS, Items.MELON_SEEDS, Items.PUMPKIN_SEEDS, Items.BEETROOT_SEEDS));
