...
public class TakeJobSiteTask extends Task<VillagerEntity> {
   ...
   protected void run(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      ...
      if (optional.isPresent()) {
         LookTargetUtil.streamSeenVillagers(villagerEntity, (villagerEntityx) -> {
            return this.canUseJobSite((PointOfInterestType)optional.get(), villagerEntityx, blockPos);
         }).findFirst().ifPresent((villagerEntity2) -> {
            this.claimSite(serverWorld, villagerEntity, villagerEntity2, blockPos, villagerEntity2.getBrain().getOptionalMemory(MemoryModuleType.JOB_SITE).isPresent());
         });
      }
   }
   ...
   private void claimSite(ServerWorld world, VillagerEntity previousOwner, VillagerEntity newOwner, BlockPos pos, boolean jobSitePresent) {
      ...
      if (!jobSitePresent) {
         LookTargetUtil.walkTowards(newOwner, (BlockPos)pos, this.speed, 1);
         newOwner.getBrain().remember(MemoryModuleType.POTENTIAL_JOB_SITE, (Object)GlobalPos.create(world.getRegistryKey(), pos));
