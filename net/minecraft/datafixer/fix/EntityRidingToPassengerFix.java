...
public class EntityRidingToPassengerFix extends DataFix {
   ...
   private <OldEntityTree, NewEntityTree, Entity> TypeRewriteRule fixEntityTree(Schema inputSchema, Schema outputSchema, Type<OldEntityTree> inputEntityTreeType, Type<NewEntityTree> outputEntityTreeType, Type<Entity> inputEntityType) {
      Type<Pair<String, Pair<Either<OldEntityTree, Unit>, Entity>>> type = DSL.named(TypeReferences.ENTITY_TREE.typeName(), DSL.and(DSL.optional((Type)DSL.field("Riding", inputEntityTreeType)), inputEntityType));
      Type<Pair<String, Pair<Either<List<NewEntityTree>, Unit>, Entity>>> type2 = DSL.named(TypeReferences.ENTITY_TREE.typeName(), DSL.and(DSL.optional((Type)DSL.field("Passengers", (Type)DSL.list(outputEntityTreeType))), inputEntityType));
            ...
            return (pair) -> {
               ...
               while(true) {
                  Either<List<NewEntityTree>, Unit> either = (Either)DataFixUtils.orElse(optional.map((pairx) -> {
                     Typed<NewEntityTree> typed = (Typed)outputEntityTreeType.pointTyped(dynamicOps).orElseThrow(() -> {
                        return new IllegalStateException("Could not create new entity tree");
                     });
                     NewEntityTree object = typed.set(opticFinder2, (Object)pairx).getOptional(opticFinder3).orElseThrow(() -> {
