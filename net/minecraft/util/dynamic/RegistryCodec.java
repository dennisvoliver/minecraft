...
public final class RegistryCodec<E> implements Codec<SimpleRegistry<E>> {
   ...
   public <T> DataResult<Pair<SimpleRegistry<E>, T>> decode(DynamicOps<T> ops, T input) {
      ...
      return ops instanceof RegistryOps ? dataResult.flatMap((pair) -> {
         return ((RegistryOps)ops).loadToRegistry((SimpleRegistry)pair.getFirst(), this.registryRef, this.elementCodec).map((simpleRegistry) -> {
