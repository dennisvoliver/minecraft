...
public class RealmsBackupScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.downloadButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width - 135, row(1), 120, 20, new TranslatableText("mco.backup.button.download"), (buttonWidget) -> {
         this.downloadClicked();
      }));
      this.restoreButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width - 135, row(3), 120, 20, new TranslatableText("mco.backup.button.restore"), (buttonWidget) -> {
         this.restoreClicked(this.selectedBackup);
      }));
      this.changesButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width - 135, row(5), 120, 20, new TranslatableText("mco.backup.changes.tooltip"), (buttonWidget) -> {
         this.client.openScreen(new RealmsBackupInfoScreen(this, (Backup)this.backups.get(this.selectedBackup)));
         this.selectedBackup = -1;
      }));
      this.addButton(new ButtonWidget(this.width - 100, this.height - 35, 85, 20, ScreenTexts.BACK, (buttonWidget) -> {
         this.client.openScreen(this.parent);
      }));
      this.addChild(this.backupObjectSelectionList);
      this.titleLabel = (RealmsLabel)this.addChild(new RealmsLabel(new TranslatableText("mco.configure.world.backup"), this.width / 2, 12, 16777215));
   ...
   private void restoreClicked(int selectedBackup) {
      if (selectedBackup >= 0 && selectedBackup < this.backups.size() && !this.serverData.expired) {
         ...
         Text text = new TranslatableText("mco.configure.world.restore.question.line1", new Object[]{string, string2});
   ...
   private void downloadWorldData() {
      this.client.openScreen(new RealmsLongRunningMcoTaskScreen(this.parent.getNewScreen(), new DownloadTask(this.serverData.id, this.slotId, this.serverData.name + " (" + ((RealmsWorldOptions)this.serverData.slots.get(this.serverData.activeSlot)).getSlotName(this.serverData.activeSlot) + ")", this)));
   }
